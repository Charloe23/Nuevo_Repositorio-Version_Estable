    ...
    void endpointsMProg(void *pvParameters) {
        ...
        server.on("/leer-parametros", HTTP_GET, [](AsyncWebServerRequest *request){
            ...
            doc["codigoSensor"] = activo.codigoSensor;
            doc["codigoPrueba"] = activo.codigoPrueba;
    ...
    if (request->hasParam("id", true) && 
        request->hasParam("zona", true) && 
        request->hasParam("tipo", true) &&
        request->hasParam("codigoSensor", true) &&
        request->hasParam("codigoPrueba", true)) {
        ...
        long nuevoCodigoSensor = request->getParam("codigoSensor", true)->value().toInt();
        long nuevoCodigoPrueba = request->getParam("codigoPrueba", true)->value().toInt();
        ...
        if(nuevoCodigoSensor < 10000000 || nuevoCodigoSensor > 99999999) {
            request->send(400, "text/plain", "Código RF Sensor debe ser de 8 dígitos");
            return;
        }

        if(nuevoCodigoPrueba < 10000000 || nuevoCodigoPrueba > 99999999) {
            request->send(400, "text/plain", "Código RF Prueba debe ser de 8 dígitos");
            return;
        }
        ...
        activo.codigoSensor = nuevoCodigoSensor;
        activo.codigoPrueba = nuevoCodigoPrueba;
        ...
        // Respuesta exitosa
        String response = "{\"status\":\"success\",\"message\":\"Configuración guardada correctamente\"}";
        request->send(200, "application/json", response);
        ...
        Serial.println("Código RF Sensor: " + String(activo.codigoSensor));
        Serial.println("Código RF Prueba: " + String(activo.codigoPrueba));
        
        mostrarImagen(img2);
    } else {
        request->send(400, "text/plain", "Faltan parámetros requeridos");
    }
});
        server.on("/enviar-rf-prueba", HTTP_POST, [](AsyncWebServerRequest *request){
            ...
            doc["codigo"] = activo.codigoPrueba;
            doc["bits"] = RF_BITS;
            ...
            Serial.println("\n[PRUEBA] Señal RF enviada. Código: " + String(activo.codigoPrueba));
        });
        ...
        server.on("/enviar-rf-alerta", HTTP_POST, [](AsyncWebServerRequest *request){
            ...
            doc["codigo"] = activo.codigoSensor;
            doc["bits"] = RF_BITS;
            ...
            Serial.println("\n[ALERTA] Señal RF enviada. Código: " + String(activo.codigoSensor));
        });
